...
const Slider: React.FC<SliderProps> = ({ slides, interval = 5000 }) => {
   ...
   const themeClasses = {
    ...
    // Change the dark mode background from gray-800 to zinc-900 for better black theme
    card: isDarkMode ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-gray-200',
};
...
return (
    <div className="w-full relative">
                                ...
                                <div className="flex flex-col sm:flex-row items-center sm:items-center">
                                    ...
                                    <div className="slider-right ml-0 sm:ml-4 md:ml-5 lg:ml-6 mt-3 sm:mt-0 flex flex-col space-y-1 sm:space-y-2 md:space-y-2.5 lg:space-y-3 w-full text-center sm:text-left">
                                        ...
                                        <div className="slider-button">
                                            <button className={`button ${isDarkMode ? 'bg-green-950 text-green-200' : 'bg-[#EBFDEB] text-green-800'} leading-[145%] p-[5px] xs:p-[6px] sm:p-[6px] md:p-[7px] lg:p-[8px] gap-[8px] rounded-full font-sora font-light text-[10px] xs:text-[11px] sm:text-[11px] md:text-[12px] inline-block`}>
                                                {slide.points}
                                            </button>
                                        </div>
        ...
        {/* Slide indicators - Update the background color for dark mode */}
        <div className="absolute bottom-0 left-0 right-0 flex justify-center">
            <div className={`flex items-center space-x-0 ${isDarkMode ? 'bg-black/80' : 'bg-white/80'} px-2.5 sm:px-3 md:px-3 py-1`}>
                <div>
                    <button
                        ...
                        <React.Fragment key={index}>
                            <button
                                ...
                                className={clsx(
                                    ...
                                    index === currentSlide ? 'bg-green-600' : isDarkMode ? 'bg-zinc-600' : 'bg-gray-300'
                                )}
